import {
  ...
  import { Heart, MapPin, Trash } from "lucide-react-native";
...
const getLocation = (setLocation) => {
  ...
  const LocationsPosted = ({navigation}) => {
    ...
    useEffect(() => {
      const fetchPosts = async () => {
        ...
        const result = await db.getAllAsync("SELECT * FROM posts WHERE user_id = ? AND location = ?", [Number(user_id), location])
        setPosts(result)
      }
      fetchPosts()
    },[navigation,posts])
  ...
  export  {LocationsPosted, getLocation};
